...
// try endpoints that match your ReasonMaintenance naming (fallbacks)
async function fetchReasons() {
  const endpoints = [
    'http://localhost:8000/api/reasonmaintances/',
    'http://localhost:8000/api/reasonmaintenances/',
    'http://localhost:8000/api/reason_maintances/',
    'http://localhost:8000/api/reason_maintenance/',
  ]
...
async function saveEdit() {
  if (!props.cardId) {
    emit('error', 'Missing maintenance id')
    return
  }
  saving.value = true
  try {
    ...
    if (editReasonId.value !== '' && editReasonId.value !== null) {
      ...
      payload.reason = Number.isNaN(parsed) ? editReasonId.value : parsed
    }
    const res = await axios.patch(
      `http://localhost:8000/api/maintenances/${props.cardId}/`,
      payload,
    ...
    emit('error', 'Erro ao salvar manutenção.')
  } finally {
    saving.value = false
  }
}
...
<template>
  <div class="maintenance-card" :class="{ editing: editing }">
    ...
    <template v-else>
      ...
      <label for="maintenance-reason" class="sr-only">Motivo</label>
      <div v-if="reasons.length">
        <select id="maintenance-reason" class="inline-input" v-model="editReasonId">
...
/* reserve top space while editing */
.maintenance-card.editing { padding-top: 2.2rem; }
...
@media (max-width: 500px) {
  ...
  .card-actions { position: static; display:flex; justify-content:space-around; width:100%; margin-bottom:0.45rem; }
