...
async function submitInspection() {
  ...
  try {
    ...
    success.value = 'Inspeção inserida com sucesso!'
    train.value = ''
    reason.value = ''
    date.value = ''
    notes.value = ''
  } catch (e: any) {
    error.value = e.response?.data?.error || 'Erro ao inserir inspeção.'
  }
}
...
<template>
  <div class="insert-inspection">
    <h2>Inserir Inspeção</h2>
